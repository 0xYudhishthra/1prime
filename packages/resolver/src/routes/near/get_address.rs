use crate::{agent::{agent_account_id, AgentConfig}, near::utils::delete_near_account, routes::agentAccount::get_agent_account, utils::get_tee_account};
use std::{str::FromStr, sync::{Arc, RwLock}};
use borsh::BorshDeserialize as _;
use k256::sha2::Sha256;
use lazy_static::lazy_static;
use near_api::{Account, Contract, Data};
...
lazy_static! {
    ...
    static ref HOLDING_NEAR_ADDRESS: Arc<RwLock<String>> = Arc::new(RwLock::new(String::new()));
    static ref HOLDING_NEAR_PUBLIC_KEY: Arc<RwLock<String>> = Arc::new(RwLock::new(String::new()));
}
...
pub fn update_holding_near_address(value: String) {
    let mut holding_near_address = HOLDING_NEAR_ADDRESS.write().unwrap();
    *holding_near_address = value;
}

pub fn update_holding_near_public_key(value: String) {
    let mut holding_near_public_key = HOLDING_NEAR_PUBLIC_KEY.write().unwrap();
    *holding_near_public_key = value;
}
...
pub async fn get_holding_near_address() -> String {
    let holding_near_address = HOLDING_NEAR_ADDRESS.read().unwrap();
    holding_near_address.clone()
}

pub async fn get_holding_near_public_key() -> String {
    let holding_near_public_key = HOLDING_NEAR_PUBLIC_KEY.read().unwrap();
    holding_near_public_key.clone()
}

pub async fn setup_holding_near_address() {
    ...
    update_holding_near_public_key(public_key_str.clone());
    ...
    // delete the account
    update_holding_near_address(near_address);
}
