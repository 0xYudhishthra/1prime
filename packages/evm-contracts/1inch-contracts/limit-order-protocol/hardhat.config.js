...
// Override Sepolia network configuration for Tatum compatibility
require('dotenv').config();
if (process.env.SEPOLIA_RPC_URL && process.env.SEPOLIA_PRIVATE_KEY && process.env.SEPOLIA_ETHERSCAN_KEY) {
    const [url, apiKey] = process.env.SEPOLIA_RPC_URL.split('|');
    networks.sepolia = {
        url: url,
    ...
    etherscan.apiKey.sepolia = process.env.SEPOLIA_ETHERSCAN_KEY;
}
