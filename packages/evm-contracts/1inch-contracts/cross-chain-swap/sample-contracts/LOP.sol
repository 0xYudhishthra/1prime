...
library MakerTraitsLib {
  ...
  function needCheckEpochManager(
    ...
    return
      (MakerTraits.unwrap(makerTraits) & _NEED_CHECK_EPOCH_MANAGER_FLAG) != 0;
  }
...
library TakerTraitsLib {
  ...
  function argsInteractionLength(
    ...
    return
      (TakerTraits.unwrap(takerTraits) >> _ARGS_INTERACTION_LENGTH_OFFSET) &
  ...
  PermitAndCall
{
      ...
      if (!takerTraits.skipMakerPermit()) {
        ...
        if (makerPermit.length >= 20) {
          ...
          if (!order.makerTraits.useBitInvalidator()) {
            ...
            if (
              !_remainingInvalidator[order.maker.get()][orderHash].isNewOrder()
    ...
    returns (uint256 makingAmount, uint256 takingAmount, bytes32 orderHash)
  {
    ...
    if (remainingMakingAmount == order.makingAmount) {
      if (!ECDSA.isValidSignature(order.maker.get(), orderHash, signature))
        revert BadSignature();
    }
  ...
  function _curfe(
    ...
    assembly ("memory-safe") {
      ...
      function tokenBalanceOf(tokenAddress, accountAddress) -> tokenBalance {
        ...
        if iszero(
          callReturnSize(staticcall(gas(), tokenAddress, 0, 0x24, 0, 0x20))
        ) {
          revert(0, 0)
        }
        tokenBalance := mload(0)
      ...
      default {
        ret := mload(
          mul(0x20, and(shr(_CURVE_SWAP_USE_SECOND_OUTPUT_OFFSET, dex), 0x01))
        )
      }
